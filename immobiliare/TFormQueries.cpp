...
{
    { "  Incontro tra domanda ed offerta di beni immobili   ", qcSupplyAndDemand },
	{ "  Valore medio di mercato degli immobili (in Euro/mq)   ", qcMeanSalesPrice },
	{ "  Istogramma degli andamenti annuali delle compravendite"   , qcSalesHistogram }
};
...
* @brief	Constructor
******************************************************************************/
__fastcall TFormQueries::TFormQueries(TComponent* Owner,
	...
	else if( g_QueryChoices[ListBox->ItemIndex].nQuery == qcSupplyAndDemand )
    {
        if( bool(m_pDB->GetAssets()) && bool(m_pDB->GetCustomers()) )
        {
            TFormQuerySupplyAndDemand *pFormQuery =
            	new TFormQuerySupplyAndDemand(NULL, m_pDB, m_hWndMsgs);
            assert(pFormQuery);
