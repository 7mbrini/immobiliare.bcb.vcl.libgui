/*!****************************************************************************
	...
	@noop	last update: 8/11/2021, 22/11/2021, 28/11/2021
...
void FilterByCity(TVecPtrAssets& Assets, std::string strCityName, bool bFullMatch)
{
    ...
    else
    {
        for(int i=0; i<TempQuery.size(); i++)
        {
            if( utils::LowerCase(std::string(TempQuery[i]->m_strCity))
            	.find( utils::LowerCase(strCityName)) != std::string::npos )
            {
                Assets.push_back(TempQuery[i]);
            }
        }
...
void FilterByAddress(TVecPtrAssets& Assets, std::string strAddress, bool bFullMatch)
{
    ...
    else
    {
        for(int i=0; i<TempQuery.size(); i++)
        {
            if( utils::LowerCase(std::string(TempQuery[i]->m_strAddress))
            	.find( utils::LowerCase(strAddress) ) != std::string::npos  )
            {
                Assets.push_back(TempQuery[i]);
            }
        }
