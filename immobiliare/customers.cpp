	...
	std::string strFirstName, bool bFullMatch)
{
    ...
    else
    {
        ...
        {
            if( utils::LowerCase(std::string(TempQuery[i]->m_strFirstName))
            	.find( utils::LowerCase(strFirstName) ) != std::string::npos  )
            {
                Customers.push_back(TempQuery[i]);
            }
        }
...
void FilterByLastName(TVecPtrCustomers& Customers, std::string strLastName, bool bFullMatch)
{
    ...
    else
    {
        for(int i=0; i<TempQuery.size(); i++)
        {
            if( utils::LowerCase(std::string(TempQuery[i]->m_strLastName))
            	.find( utils::LowerCase(strLastName) ) != std::string::npos  )
            {
                Customers.push_back(TempQuery[i]);
            }
        }
    ...
    else
    {
        for(int i=0; i<TempQuery.size(); i++)
        {
            if( utils::LowerCase(std::string(TempQuery[i]->m_strCity))
            	.find( utils::LowerCase(strCity) ) != std::string::npos  )
            {
                Customers.push_back(TempQuery[i]);
            }
        }
